...
options.html/.js       options page: the passive donation block
lib/config.js          the one place the donation URL lives
lib/donate.js          donate visibility rules (pure, unit tested)
...
test/donate.test.js    node --test
build.ps1              builds dist/attach-and-reply-<version>.xpi
tools/make-icons.py    regenerates the icons (standard library only)
```

The options page has no settings on purpose. The only thing on it is the
donation block; the add-on itself takes no configuration.
...
Donations are one of the three monetisation options Mozilla explicitly allows for
add-ons, and this add-on asks in the least intrusive way it can: **nothing is
ever pushed at the user**. The ask is a card on the options page, and "I already
donated" hides it for 90 days.

There is exactly **one** URL to maintain:
...
- **Stripe's** supported-country list contains Hong Kong SAR and Macao SAR but
  **not mainland China**, so every Stripe-payout platform (Ko-fi, Buy Me a
