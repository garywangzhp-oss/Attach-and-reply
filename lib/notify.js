...
/** The original message could not be read at all (not downloaded, or the API failed). */
export async function showNotDownloadedNotice({ browser, log, tabId }) {
  const action = actionApi(browser);
  if (!action) {
    return;
  }
  ...
  try {
    ...
    await action.setBadgeBackgroundColor(details({ color: BADGE_COLOR }));
    await action.setTitle(details({ title: message(browser, "noticeNotDownloaded") }));
  } catch (error) {
    log?.warn?.("could not update the toolbar button", error);
  }
}
